...
- **Wave 9 검증**: 이든 직접 통합 스모크 = llm_client engine 기본 cli·api 키부재 LLMError 확인, scope 필터 실측(private만 제외·shared/미지정 포함). E1(wiki_app 2경로 mock 관측)·E2(main dry-run private=1 관측) 각자 E2E + 유닛 27 신규.
- **Codex 적대리뷰 회수 (2026-07-04, `/codex:adversarial-review` shared runtime — mailbox 대신 companion 직접호출로 정상 응답)**: v0.3 전체(d9d3bf7 대비) 적대 검증 → **high 2건 포착·수정**(`cbc9feb`). 발견: `engine: api` 경로가 cli 의 timeout 보호를 우회 — ⑴ `_call_api` 비스트림이 요청 timeout 미적용 ⑵ `_stream_api` 가 `_open/_next` 를 timeout 없이 await → Anthropic hang 시 wiki_app 요청/worker 무기한 점유. 수정: api 분기에 SDK client `timeout=` + `asyncio.wait_for` 이중 적용(cli 와 동일 `asyncio.TimeoutError` 계약) + hang mock 회귀 3(코드만 revert 시 FAIL 로 반증). **교훈**: E1 유닛·E2E 가 정상응답 mock 만 봐 못 잡음 = `feedback_safety_not_function`(mock 이 실경로 가림). 적대 렌즈 정상화가 실제 ship-blocker 포착. 512 통과.
